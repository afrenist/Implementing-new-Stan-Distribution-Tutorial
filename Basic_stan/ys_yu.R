setwd(file.path(here::here(), 'Basic_stan'))
##############################
...
# set the home dir
home.dir <- '/Users/yu'
set_cmdstan_path( file.path(home.dir, ".cmdstan/cmdstan-2.36.0"))

current_version <- cmdstanr::cmdstan_version()
cat("Current Stan version:", current_version, "\n")

# Update to 2.36.0 if not already using it
if (current_version != "2.36.0") {
  install_cmdstan(version = "2.36.0", overwrite = T)
}
if( packageVersion('cmdstanr') < '0.8.1' )
{
  remotes::install_github("stan-dev/cmdstanr", ref = "v0.8.1")
}
